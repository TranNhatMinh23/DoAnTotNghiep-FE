...
class ContactProcessedTable extends Component {
  state = {
    displayDelete: false,
    displayDetail: false,
    detailInfo: null,
  ...
  render() {
    const { listProcessedContacts, t } = this.props;
    ...
    return (
      <Fragment>
        <Table 
          rowKey="id" 
          dataSource={listProcessedContacts} 
        ...
        >
          ...
          <Column
            ...
            render={(status, row) => (
              <span key={status}>
                <Tag className="hover-tag" color="red" key={row.id}>
                  {t('processed')}
                </Tag>
              </span>
            )}
...
export default withTranslation()(ContactProcessedTable);