...
import styles from './RenderQuestionPartSeven.module.css'; 
import { updateQuestionTextInList, getAnswerKey, getItemById } from '../../../../../../../shared/function';
import GroupButtonAndInpuForFirst from './GroupButtonAndInpuForFirst';
...
class RenderQuestionPartSeven extends PureComponent {
  ...
  onChangeStatusIsGroupQuestion = (questionId) => {
    this.props.changeStateIsFirstGroupQuestion(questionId);
  }

  onChangeGroupDescription = (e, questionId) => {
    let objIndex = this.state.objQuestion.findIndex((obj => obj.id === questionId));
    let tempObjQuestions = this.state.objQuestion;
    tempObjQuestions[objIndex].group_desc = e.target.value;
    this.setState({
      objQuestion: tempObjQuestions,
    })
  }
  ...
  onSaveDataAPI = (id) => {
    ...
    if(question.isFirstGroup){
      data.group_desc = question.group_desc;
    } else {
        ...
        {listQuestions.map(question => {
          return (
            <WidgetQuestion key={question.id}>
              <div className={styles.checkboxIsFirstGroup}>
                <Checkbox
                  defaultChecked={question.isFirstGroup}
                  onChange={() => this.onChangeStatusIsGroupQuestion(question.id)}
                >
                  {t('isFirstGroup')}
                </Checkbox>
                {
                  question.isFirstGroup &&
                  <GroupButtonAndInpuForFirst
                    question={question}
                    onChangeGroupDescription={(e) => this.onChangeGroupDescription(e, question.id)}
                  />
                }
              </div>
...
const mapDispatchToProps = dispatch => ({
  ...
  changeStateIsFirstGroupQuestion: (questionId) => dispatch(actions.changeStateIsFirstGroupQuestion(questionId))
});

export default connect(mapStateToProps, mapDispatchToProps)(withTranslation()(RenderQuestionPartSeven));