...
class ScreenDetailPartFour extends Component {
  render(){
    const { detailPart, detailExamQuestion, t } = this.props;
    return (
      <WidgetPage>
        <CustomPageHeader to={`/exam-questions/${detailExamQuestion.id}`} title={`${t('detailPart')} 4 - ${detailExamQuestion.name}`} /> 
        <WidgetQuestion>
          <p className="part-name">PART {detailPart.part_no}</p>
          <p><strong>Directions: </strong>{directionText.directionPartFour}</p>
        </WidgetQuestion>
...
export default withTranslation()(ScreenDetailPartFour);