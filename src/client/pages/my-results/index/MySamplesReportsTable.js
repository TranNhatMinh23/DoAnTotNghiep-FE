...
class MySamplesReportsTable extends Component {
  constructor(props){
    super(props);
    this.state = {
      sortedInfo: null,
      searchingData: this.props.listSamplesReports
    }
  } 
  ...
  handleSearch = (searchText = '') => {
    let matchingData = searchByExamName(this.props.listSamplesReports, searchText); 
    this.setState({
  ...
  render() {
    ...
    return (
      <div className={`${styles.widgetTable} ${styles.borderTopTable}`}>
        <div className={styles.headerTableWidget}>
          <p className={styles.titleTable}>{t('reportsSampleExamsTaking')}</p>
        ...
        >
          ...
          <Column title={t('sampleExamName')} dataIndex="exam.name" key="name" />
          ...
          <Column
            ...
            render={(item, row) => (
              <span>
                ...
                &nbsp;
                <Tooltip placement="bottom" title={t('practiceAgain')}>
                  <Link to={`/preview-sample-exam?sampleExamId=${row.exam.id}`}>
                    <Icon className={styles.iconRedo} type="redo" /> 
                  </Link>
                </Tooltip>
                &nbsp;
              </span>
...
export default withTranslation()(MySamplesReportsTable);