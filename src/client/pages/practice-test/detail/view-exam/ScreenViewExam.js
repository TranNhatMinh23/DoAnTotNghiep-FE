...
import styles from './ScreenViewExam.module.css';
...
class ScreenViewExam extends Component {
  constructor(props) {
    super(props);
    this.state = {
      time: {},
      ...
      objectAnswer: genarateObjectAnswer((this.props.detailExam && this.props.detailExam.parts) ? this.props.detailExam.parts : []),
    }
  }
  ...
  componentWillUnmount() {
    ...
    window.removeEventListener("beforeunload", e => e.returnValue = '');
  }
  ...
  submitAnswer = () => {
    let examId = this.props.detailExam.exam_id;
    let data = {
  ...
  render() {
    ...
    const { detailExam, isLoading, t } = this.props;

    return (
      <DocumentTitle title={`${detailExam.exam_name}`}>
        <div className={`${styles.viewExam}`}>
          <div id="TopExam"></div>
          <div className={`preventcopy ${styles.mainContentOfExam}`}>
            <div className={styles.titleExam}>
              <span>{detailExam.exam_name}</span>
            </div>
            <div className={`test-content ${styles.mainViewOfContentExam}`}>
              <ViewPartOne
                audio={detailExam.audio}
                listQuestionPart1={detailExam.parts[0]}
                onChangeAnswer={this.onChangeAnswer}
              />
              <ViewPartTwo
                listQuestionPart2={detailExam.parts[1]}
                onChangeAnswer={this.onChangeAnswer}
              />
              <ViewPartThree
                listQuestionPart3={detailExam.parts[2]}
                onChangeAnswer={this.onChangeAnswer}
              />
              <ViewPartFour
                listQuestionPart4={detailExam.parts[3]}
                onChangeAnswer={this.onChangeAnswer}
              />
              <ViewPartFive
                listQuestionPart5={detailExam.parts[4]}
                onChangeAnswer={this.onChangeAnswer}
              />
              <ViewPartSix
                listQuestionPart6={detailExam.parts[5]}
                onChangeAnswer={this.onChangeAnswer}
              />
              <ViewPartSeven
                listQuestionPart7={detailExam.parts[6]}
                onChangeAnswer={this.onChangeAnswer}
              />
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(withTranslation()(ScreenViewExam)));
